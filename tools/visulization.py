...
def visualize_test_evaluation(input_file, output_folder, experiment_title = ''):
    ...
    plt.text(0.4 * len(dsc_avg_list) * 10, 0.75, 'max. dice coefficient: ' + str(np.max(dsc_avg_list)))
    plt.text(0.4 * len(dsc_avg_list) * 10, 0.70, 'max. volume similarity: ' + str(np.max(vs_avg_list)))
