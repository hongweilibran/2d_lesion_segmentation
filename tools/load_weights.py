...
def main():
    ...
    old_model.load_weights('~/models/Full_Flair_T1_5_Augmentation/0.h5')

    model = get_model(img_shape=(240, 240, 2), num_classes=9)

    for ind, layer in enumerate(old_model.layers[1:17]):
        if layer.trainable:
            model.layers[ind+1].set_weights(layer.get_weights())

    model.save_weights('pretrained_models/pretrained_unet.h5')
