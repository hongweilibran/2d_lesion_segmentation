...
def get_model(img_shape=None, num_classes=2):
    ...
    conv3_1 = Conv2D(32, (3, 3), dilation_rate=4, padding='same', activation='relu')(pool2)
    conv3_skip = Conv2D(32, (1, 1), dilation_rate=4, padding='same', activation='relu')(pool2)
    conv3_2 = Conv2D(32, (3, 3), dilation_rate=4, padding='same', activation='relu')(conv3_1)
    add3 = Add()([conv3_2, conv3_skip])
    pool3 = MaxPooling2D(pool_size=(2, 2))(add3)

    conv4_1 = Conv2D(32, (3, 3), dilation_rate=8, padding='same', activation='relu')(pool3)
    conv4_skip = Conv2D(32, (1, 1), dilation_rate=8, padding='same', activation='relu')(pool3)
    conv4_2 = Conv2D(32, (3, 3), dilation_rate=8, padding='same', activation='relu')(conv4_1)
    add4 = Add()([conv4_2, conv4_skip])
    upsamp4 = UpSampling2D(size=(2, 2))(add4)

    skip_concat1 = Concatenate()([upsamp4, add3])

    conv5_1 = Conv2D(32, (3, 3), dilation_rate=4, padding='same', activation='relu')(skip_concat1)
    conv5_skip = Conv2D(32, (1,1), dilation_rate=4, padding='same', activation='relu')(skip_concat1)
    conv5_2 = Conv2D(32, (3, 3), dilation_rate=4, padding='same', activation='relu')(conv5_1)
